...
class PublisherJointTrajectory(Node):
    def __init__(self):
        ...
        if self.check_starting_point:
            ...
            for name in self.joints:
                ...
                self.get_logger().info(param_name_tmp + " has limit " + str(self.starting_point[name]))
    ...
    def joint_state_callback(self, msg):
        ...
        if self.controller_name == "joint_trajectory_position_controller":
            ...
            for idx, enum in enumerate(msg.name):
                if append:
                    ...
                    append = True
                positionMsg += str(msg.name[idx]) + " " + str(round(msg.position[idx], 3))
                velocityMsg += str(msg.name[idx]) + " " + str(round(msg.velocity[idx], 3))

            self.get_logger().info(positionMsg.replace("_joint", ""))
            self.get_logger().info(velocityMsg.replace("_joint", ""))
        ...
        if not self.joint_state_msg_received:
            ...
            for idx, enum in enumerate(msg.name):
                if enum in self.starting_point:
